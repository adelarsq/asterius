...
mkInfoTable dflags proc@(CmmProc _ entry_lbl live blocks)
...
entryCode dflags e = CmmLoad e (bWord dflags)
...
infoTable dflags info_ptr = cmmOffsetW dflags info_ptr 1 -- Past the entry code pointer
...
-- in the info table.
funInfoTable dflags info_ptr = cmmOffsetW dflags info_ptr (1 + stdInfoTableSizeW dflags)
...
funInfoArity dflags iptr
  ...
  where
   ...
   (rep_bytes, offset) = ( pc_REP_StgFunInfoExtraFwd_arity pc
